...
namespace NotHotdog.iOS {
	...
	/// </summary>
	public class NotHotDogInput : NSObject, IMLFeatureProvider
	{
		...
		public NotHotDogInput (CVPixelBuffer data)
		{
			if (data == null)
	...
	/// </summary>
	public class NotHotDogOutput : NSObject, IMLFeatureProvider
	{
		static readonly NSSet<NSString> featureNames = new NSSet<NSString> (
		...
		public NotHotDogOutput (NSDictionary<NSObject, NSNumber> loss, string classLabel)
		{
			if (loss == null)
	...
	/// </summary>
	public class NotHotDog : NSObject
	{
		readonly MLModel model;

		public NotHotDog ()
		{
			var url = NSBundle.MainBundle.GetUrlForResource ("NotHotDog", "mlmodelc");
			NSError err;

			model = MLModel.Create (url, out err);
		}

		NotHotDog (MLModel model)
		{
			this.model = model;
		}

		public static NotHotDog Create (NSUrl url, out NSError error)
		{
			if (url == null)
			...
			return new NotHotDog (model);
		}

		/// <summary>
		/// Make a prediction using the standard interface
		/// </summary>
		/// <param name="input">an instance of NotHotDogInput to predict from</param>
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public NotHotDogOutput GetPrediction (NotHotDogInput input, out NSError error)
		{
			...
			return new NotHotDogOutput (lossValue, classLabelValue);
		}
		...
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public NotHotDogOutput GetPrediction (CVPixelBuffer data, out NSError error)
		{
			var input = new NotHotDogInput (data);
